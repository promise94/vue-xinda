                ...
                <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
                    <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
                        <div>
                            ...
                            <span class="text" v-if="child.eventName!=undefined">{{child.eventName}}</span>
                            <span class="text" :class="{'isLunarFestival':child.isLunarFestival,'isGregorianFestival':child.isGregorianFestival}" v-if="lunar">{{child.lunar}}</span>
                        </div>
                    </td>
                </tr>
            </tbody>
...
.calendar td:not(.selected) div:hover,
.calendar td:not(.selected) div:hover span {
    background: orange;
    color: #fff;
}
